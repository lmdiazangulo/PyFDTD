...
print(totalFieldBox_len)
delay  = 8e-9
spread = 2e-9
# Indexes of the total field box limits in the E grid
tFB_Eidx = np.array((
        (np.abs(gridE - totalFieldBox_lim[0])).argmin(),
        (np.abs(gridE - totalFieldBox_lim[1])).argmin()))
...
for n in range(numberOfTimeSteps):
    # --- Updates E field ---
    for i in xrange(1, gridE.size-1):
        eNew[i] = eOld[i] + cE * (hOld[i-1] - hOld[i])

    # Ilumination
    eNew[tFB_Eidx[0]] += gaussianFunction(dt*n, delay, spread)
    eNew[tFB_Eidx[1]] -= gaussianFunction(
            dt*n + totalFieldBox_len/c0, delay, spread)
...
#    eNew[0] = eOld[0] + cE * (hOld[-1] - hOld[0])
    ...
    # Ilumination
    hNew[tFB_Eidx[0]-1] += gaussianFunction(dt*n, delay, spread)/impedance
    hNew[tFB_Eidx[1]-1] -= 10
   # gaussianFunction(
   #         dt*n + totalFieldBox_len/c0, delay, spread)/impedance
